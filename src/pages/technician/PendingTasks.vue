...
const sortOptions = [
  { value: 'date_asc', label: 'วันดำเนินการที่ใกล้ถึง' },
  { value: 'date_desc', label: 'รายการล่าสุด' }
];
...
const displayOrders = computed(() => {
    let results = allJobs.value.map(job => {
    ...
    results.sort((a, b) => {
        ...
        return a.rawDate - b.rawDate; // Default date_asc
    });
...
<template>
  <div class="flex flex-col gap-6 w-full max-w-5xl mx-auto bg-white p-6 rounded-lg shadow-sm">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <h1 class="style-headline-2 text-gray-900">รายการที่รอดำเนินการ</h1>
    ...
    <div class="flex flex-wrap items-center gap-6 mt-2 mb-2">
      ...
      <div class="flex items-center gap-3">
        <span class="style-body-2 text-gray-700">เรียงตาม</span>
        <div class="relative">
          <select v-model="sortOrder" class="appearance-none border border-gray-200 rounded-[8px] pl-3 pr-8 py-2 style-body-2 text-gray-700 bg-white outline-none focus:border-blue-500 cursor-pointer min-w-[160px]">
